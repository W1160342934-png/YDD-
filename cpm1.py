...
try:
    ...
    print("[错误] 找不到 cylo.py 文件。请确保它和 cpm1.py 在同一目录下。")
    sys.exit(1)
...
def banner(console):
    ...
    # 为左对齐的艺术图案应用渐变
    colored_art = gradient_text_multi_line(ascii_art_jbc_left.strip(), art_colors) # 使用修改后的爱心图案
    # 左对齐打印图案 (默认对齐方式就是左对齐)
    console.print(colored_art)
    console.print("\n")
    # --- ASCII 艺术结束 ---

    # 更新品牌名称并左对齐 (修改标识：季伯常专属工具 → 大爱仙尊专属工具)
    ...
    console.print(random_gradient_text_line_rich("独自问鼎 YDD 私信获得工具箱安装教程及使用权！"))
...
if __name__ == "__main__":
    ...
    while True: # 登录循环
        ...
        # 删除 Access Key 输入步骤
        gradient_separator("账号登录", console, separator_char=SEPARATOR_CHAR, start_color=PINK, end_color=PURPLE)
        acc_email = prompt_valid_value("请输入账号邮箱", "邮箱", console, password=False)
        acc_password = prompt_valid_value("请输入账号密码", "密码", console, password=False)

        console.print("[bold yellow][%] 正在尝试登录...", end="")

        try:
            # 初始化 Bubcyz 类时移除 access key 参数（需确保 cylo.py 中 Bubcyz 类支持无参初始化）
            cpm = Bubcyz()
        ...
        while True:
            ...
            # 定义菜单选项数据（删除 Access Key 相关功能，保留原39项功能）
            menu_options_data = [
